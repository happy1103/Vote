...
## 2026-09-14 更新
- 同一瀏覽器中的主持人身份與一般投票者身份已分開計算，因此同一支手機同時以主持人頁與投票頁參與時，會算作 2 位參與者／2 張票。
- 若最高票同票，主持人在按下「顯示結果」的當下由瀏覽器安全亂數自動抽出一位晉級者；主持人無法預先指定或預知抽籤結果。公開票數仍保留實際同票票數。
- 更新後請同步發布新的 `firestore.rules`。
- 建議用「新建立的場次」測試新版身份計算，舊版場次可能保留舊格式的 participant/vote 文件。

## 2026-09-14 更新
- 投票者在第 1～5 階段結束後不會看到該階段的公開票數或勝出結果，只會等待下一階段。
- 主持人仍可在每一階段按「顯示結果」查看票數與比例。
